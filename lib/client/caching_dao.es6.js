    ...
    },
    {
      ...
      code: function(array) {
        return new Promise(function(resolve, reject) {
          return this.cacheBatch_(resolve, reject, array, 0);
        }.bind(this));
      },
    },
    {
      name: 'cacheBatch_',
      documentation: `Put a batch of array results to cache. Also asynchronously
          starts next batch.`,
      code: function(resolve, reject, array, skip) {
        try {
          ...
          for (let i = skip; i < array.length; i++) {
            ...
            if (i % batchSize === mod) {
              setTimeout(this.cacheBatch_.bind(this, resolve, reject, array,
                                               i + 1),
                         this.batchDelay);
              return;
            }
