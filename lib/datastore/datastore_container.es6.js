  ...
  properties: [
    ...
    },
    {
      ...
      factory: function() {
        ...
        if (!this.authAgent)
          return ctx.createSubContext(this.Context.create(null, ctx));
  ...
  values: [
    ...
    },
    {
      ...
      getServiceDAO: function(cls, versionedCls, baseRemoteDAO, ctx) {
        ...
        const getDataJournalFD = function() {
          return require('fs').openSync(
              require('path').resolve(
                  __dirname,
                  `../../data/${versionedCls.id}-journal.js`),
              'r');
        };
        const sync = this.SyncDAO.create({
          ...
          delegate: this.JDAO.create({
            of: versionedCls,
            delegate: this.MDAO.create({of: versionedCls}, ctx),
