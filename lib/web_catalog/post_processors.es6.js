...
foam.CLASS({
  name: 'RenameInterfaceProcessor',
  package: 'org.chromium.apis.web',
  implements: ['org.chromium.apis.web.ApiCatalogPostProcessor'],

  documentation: 'Rename an interface.',
  ...
  properties: [
    ...
    },
    {
      class: 'Boolean',
      name: 'overwrite',
      documentation: `Rename if "toInterfaceName" already exists?`,
    },
  ],
  ...
  methods: [
    function postProcess(apiCatalog, og) {
      ...
      // Do not rename if "fromIntefaceName" doesn't exist, or "overwrite" is
      // disabled and rename constitutes an overwrite.
      if (!apiCatalog.hasOwnProperty(this.fromInterfaceName) ||
          (apiCatalog.hasOwnProperty(this.toInterfaceName) && !this.overwrite))
        return;

      apiCatalog[this.toInterfaceName] = apiCatalog[this.fromInterfaceName];
      delete apiCatalog[this.fromInterfaceName];
    },
  ],
});

foam.CLASS({
  name: 'RemoveGlobalObjectProcessor',
  package: 'org.chromium.apis.web',
  implements: ['org.chromium.apis.web.ApiCatalogPostProcessor'],
