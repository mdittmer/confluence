...
foam.CLASS({
  ...
  package: 'org.chromium.apis.web',
  extends: 'org.chromium.apis.web.RemoveInterfacesPreprocessor',
  implements: ['org.chromium.apis.web.ApiCatalogPostProcessor'],
  ...
  methods: [
    function postProcess(apiCatalog, og) {
      ...
      for (let i = 0; i < this.interfaceNames.length; i++) {
        let iface = this.interfaceNames[i];
        if (apiCatalog[iface]) {
          let apis = apiCatalog[iface];
          for (let apiName in apis) {
            if (this.apiNameRegExp.match(apiName) !== null)
              delete apiCatalog[iface][apiName];
          }
        }
      }
    }
