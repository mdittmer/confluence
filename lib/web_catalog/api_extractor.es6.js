...
foam.CLASS({
  ...
  requires: [
    ...
    'org.chromium.apis.web.RemoveGlobalObjectProcessor',
    ...
    },
    {
      ...
      factory: function() {
        return [
          ...
          this.RemoveGlobalObjectProcessor.create(),
        ];
      },
    },
    {
  ...
  methods: [
    {
      ...
      code: function(og) {
        ...
        for (const id of allOgIds) {
          if (id === og.lookup('HTMLMediaElement') || id === og.lookup('HTMLMediaElement.prototype')) debugger;
        ...
        for (const libId in this.libs_) {
          if (this.apis_[libId] && this.apis_[libId].length > 0) {
            ...
            for (const name of names) {
              apiCatalog[name] = this.apis_[libId];
            }
          }
        }
    ...
    function storeProtoAPIsForCtor_(og, ctorId, ctorNames, protoId,
      ...
      //         "ctorId"), or
      //   (2b) "nextProtoId" is neither a library, nor some "Foo.prototype",
