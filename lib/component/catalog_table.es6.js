...
function catalogTableController($scope, api) {
  ...
  function profile(str) {
    console.log('PROFILE START', str);
    profiles[str] = performance.now();
    console.profile(str);
  }
  function profileEnd(str) {
    console.log('PROFILE END', str);
    console.profileEnd(str);
    console.log('PROFILE REPORT', str, performance.now() - profiles[str]);
  }
  let wasLoaded = !!((this.waitState & WAIT_FLAGS.DATA) &&
                     (this.waitState & WAIT_FLAGS.NETWORK));
  if (wasLoaded) profileEnd('Catalog load');
  else           profile('Catalog load');
  this.isLoaded = function() {
    let nextLoaded = !!((this.waitState & WAIT_FLAGS.DATA) &&
                        (this.waitState & WAIT_FLAGS.NETWORK));
    if (wasLoaded !== nextLoaded) {
      if (wasLoaded) profile('Catalog load');
      else           profileEnd('Catalog load');
    }

    wasLoaded = nextLoaded;
    return nextLoaded;
  };
