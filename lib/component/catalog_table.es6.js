...
function catalogTableController($scope, api) {
  ...
  let releaseKeys;
  let onChanges = function(releaseKeys) {
  ...
  ctrl.$onChanges = function(changes) {
    if (changes.releases && changes.releases.currentValue) {
      releaseKeys = getReleaseKeys(changes.releases.currentValue);
      onChanges(releaseKeys);
    }
  };

  apiMatrix.sub('on', 'reset', function() {
    console.log('Reset: toMatrix()...');
    releaseKeys && onChanges(releaseKeys);
  });
