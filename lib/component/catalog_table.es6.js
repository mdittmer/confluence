...
function catalogTableController($scope, api) {
  ...
  // TODO(markdittmer): Does this listener merging/debouncing on some interval?
  function onNewData(sub, topic, message) {
    if (foam.typeOf(message.object) !== foam.String) {
      console.warn('apiCatalogTable: Unexpected new data message:',
                   message.object);
      return;
    }
    const releaseKey = message.object;
    const releaseKeys = getReleaseKeys(ctrl.releases);
    if (releaseKeys.some(key => key === releaseKey)) {
      apiMatrix.toMatrix(releaseKeys, {
        searchKey: $scope.searchKey || undefined,
        releaseOptions: ctrl.releaseOptions,
        numAvailable: ctrl.numAvailable,
      }).then(displayMatrix);
    }
  }
  const newDataSub = require('../client/new_data_publisher.es6.js')
      .sub('message', onNewData);
  $scope.$on('$destroy', function() { newDataSub.detach(); });
}
